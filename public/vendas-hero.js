        ...
        (async () => {
            ...
            for (const ss of document.styleSheets) {
                ...
                try {
                    ...
                    if (ss.href) {
                        try {
                            const txt = await fetch(ss.href).then(r => { if (!r.ok)
                                throw 0; return r.text(); });
...
function Stage({ width = 1280, height = 720, duration = 10, background = '#f6f4ef', fps = 60, loop = true, autoplay = true, 
...
playback = null, persistKey = 'animstage-v3', children, }) {
    ...
    React.useEffect(() => {
        ...
        const canSyncSeek = typeof ReactDOM !== 'undefined' &&
    ...
    React.createElement("div", { ref: stageRef, "data-om-starter": "animations-v3", style: {
        ...
        React.createElement("div", { style: {
            ...
            React.createElement("svg", { ref: canvasRef, width: width, height: height, "data-om-exportable-video-with-duration-secs": duration, style: {
        ...
        React.createElement(PlaybackBar, { time: displayTime, actualTime: time, duration: duration, playing: playing, onPlayPause: () => setPlaying(p => !p), onReset: () => { setTime(0); }, onSeek: (t) => setTime(t), onHover: (t) => setHoverTime(t) })));
}
// ── Playback bar ────────────────────────────────────────────────────────────
...
function PlaybackBar({ time, duration, playing, onPlayPause, onReset, onSeek, onHover }) {
    ...
    return (React.createElement("div", { "data-omelette-chrome": true, style: {
        ...
        React.createElement(IconButton, { onClick: onPlayPause, title: "Play/pause (space)" }, playing ? (React.createElement("svg", { width: "14", height: "14", viewBox: "0 0 14 14", fill: "none" },
            React.createElement("rect", { x: "3", y: "2", width: "3", height: "10", fill: "currentColor" }),
            React.createElement("rect", { x: "8", y: "2", width: "3", height: "10", fill: "currentColor" }))) : (React.createElement("svg", { width: "14", height: "14", viewBox: "0 0 14 14", fill: "none" },
            React.createElement("path", { d: "M3 2l9 5-9 5V2z", fill: "currentColor" })))),
        React.createElement(IconButton, { onClick: onReset, title: "Return to start (0)" },
            React.createElement("svg", { width: "14", height: "14", viewBox: "0 0 14 14", fill: "none" },
                React.createElement("path", { d: "M3 2v10M12 2L5 7l7 5V2z", stroke: "currentColor", strokeWidth: "1.5", strokeLinejoin: "round", strokeLinecap: "round" }))),
        React.createElement("div", { style: {
                fontFamily: mono,
                fontSize: 12,
        ...
        React.createElement("div", { ref: trackRef, onMouseMove: onTrackMove, onMouseLeave: onTrackLeave, onMouseDown: onTrackDown, style: {
                flex: 1,
                height: 22,
                position: 'relative',
                cursor: 'pointer',
        ...
        typeof VideoEncoder !== 'undefined' && (React.createElement(IconButton, { title: "Export video", onClick: () => window.parent.postMessage({ type: 'omelette:request-video-export' }, '*') },
            React.createElement("svg", { width: "14", height: "14", viewBox: "0 0 14 14", fill: "none" },
                React.createElement("path", { d: "M7 2v7m0 0L4 6m3 3l3-3M2 12h10", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round", strokeLinejoin: "round" }))))));
}
function IconButton({ children, onClick, title }) {
    const [hover, setHover] = React.useState(false);
    return (React.createElement("button", { onClick: onClick, title: title, onMouseEnter: () => setHover(true), onMouseLeave: () => setHover(false), style: {
            width: 24, height: 24,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
...
function ssParse(raw) {
    if (typeof raw !== 'string' || !raw || raw.length > 16 * 1024)
        return null;
    var parsed;
    try {
    ...
    for (var i = 0; i < parsed.length; i++) {
        ...
        if (typeof s !== 'object' || s === null)
            return null;
        if (typeof s.name !== 'string' || typeof s.dur !== 'number')
            return null;
        if (!isFinite(s.dur) || s.dur <= 0 || s.dur > 300)
            return null;
    }
    return parsed;
}
...
function ppParse(raw) {
    if (typeof raw !== 'string' || !raw || raw.length > 256)
        return null;
    var parsed;
    try {
        parsed = JSON.parse(raw);
    ...
    if (parsed.mode === 'loop')
        return keys.length === 1 ? { mode: 'loop' } : null;
    if (parsed.mode === 'times') {
        if (keys.length !== 2)
            return null;
        var c = parsed.count;
        if (typeof c !== 'number' || c !== Math.floor(c) || c < 1 || c > 99)
            return null;
...
function PlaybackSync(props) {
    ...
    return React.createElement("div", { ref: ref, style: { display: 'none' } });
}
...
function useComposition() {
    ...
    if (!ctx)
        throw new Error('useComposition() must be called inside <CompositionStage>');
    return ctx;
}
    ...
    return new Proxy(table, {
        get: function (target, prop) {
            if (typeof prop !== 'string' || prop in target)
                return target[prop];
            if (CC_META[prop] || prop.indexOf('@@') === 0)
...
function CompositionClock(props) {
    ...
    return (React.createElement(CompositionContext.Provider, { value: value }, props.children));
}
function Shot(props) {
    ...
    return (React.createElement("div", { style: { position: 'absolute', inset: 0, visibility: on ? 'visible' : 'hidden' } }, props.children));
}
...
function Captions(props) {
    ...
    if (isFinite(end))
        o = Math.min(o, (end - t) / CAPTION_FADE);
    o = Math.max(0, Math.min(1, o));
    return (React.createElement("div", { "data-om-caption": true, style: Object.assign({
...
function CompositionStage(props) {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Stage, { width: width, height: height, duration: derived.total, background: bg, autoplay: autoplay, loop: loop, playback: pb },
            React.createElement(SceneSync, { raw: raw, onUpdate: setRaw }),
            typeof praw === 'string' && praw !== '' && (React.createElement(PlaybackSync, { raw: praw, onUpdate: setPraw })),
            React.createElement(CompositionClock, { derived: derived, cues: cues }, props.children),
            React.createElement(CcUnknownWatch, { unknownRef: unknownRef, badge: badge, setBadge: setBadge })),
        badge !== '' && (
        // Sibling of Stage, outside the exportable <svg>: visible in the
        // preview (and its screenshots), never in the exported video.
        React.createElement("div", { "data-om-unknown-cues": true, style: {
...
function useWatercolorLayers(painting, opts) {
    ...
    if (typeof painting !== 'function' || !kit || typeof kit.layers !== 'function')
        return null;
    try {
...
function WatercolorSheet(props) {
    ...
    if (!L) {
        return (React.createElement("div", { style: Object.assign(style, { background: '#f4f1e8', color: '#8a8270', font: '12px system-ui, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center' }) }, "watercolor-kit.js not loaded (or the painting failed to build)"));
    }
    return (React.createElement(WatercolorSheetContext.Provider, { value: L },
        React.createElement("div", { style: style, "data-om-watercolor-sheet": true },
            React.createElement("img", { src: L.paper, alt: props.alt || '', style: { position: 'absolute', left: 0, top: 0, width: '100%', height: '100%', display: 'block' } }),
            props.children)));
}
function WatercolorStroke(props) {
    ...
    return React.createElement("img", { src: src, alt: "", "data-om-watercolor-stroke": i, "data-om-stroke-kind": L.kind(i), style: style });
}
...
function WatercolorPainting(props) {
    ...
    React.useEffect(function () {
        if (!L || typeof L.warm !== 'function')
            return;
        var p = L.warm();
        if (warmed.current === p)
            return;
        var live = true;
        p.then(function () { warmed.current = p; if (live)
    ...
    if (L) {
        for (var i = 0; i < L.count; i++) {
            ...
            if (at <= 0)
                break;
            strokes.push(React.createElement(WatercolorStroke, { key: i, layers: L, index: i, at: at }));
        }
    }
    return React.createElement(WatercolorSheet, { layers: L, style: props.style, alt: props.alt }, strokes);
}
...
function WatercolorReveal(props) {
    ...
    cache.fn = painting;
    React.useEffect(function () {
        if (frames || cache.baking || !painting || !kit || typeof kit.bake !== 'function')
            return;
        cache.baking = true;
    ...
    if (frames)
        return React.createElement("img", { src: frames[i], alt: props.alt || '', style: style });
    if (!kit || !painting) {
        return (React.createElement("div", { style: Object.assign({ width: '100%', height: '100%', background: '#f4f1e8', color: '#8a8270', font: '12px system-ui, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center' }, props.style) }, "watercolor-kit.js not loaded (or no painting function)"));
    }
    if (!cache.frames[i]) {
        try {
            ...
            return (React.createElement("div", { style: Object.assign({ width: '100%', height: '100%', background: '#f4f1e8', color: '#8a8270', font: '12px system-ui, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center' }, props.style) },
                "painting too large to render (",
                String(e && e.message).slice(0, 80),
                ")"));
        }
    }
    return React.createElement("img", { src: cache.frames[i], alt: props.alt || '', style: style });
}
...
(function () {
    ...
    function Screen({ show, children }) {
        return React.createElement("div", { style: { position: "absolute", inset: 0, opacity: show, pointerEvents: "none", display: "flex", flexDirection: "column", gap: 10, padding: "14px 12px" } }, children);
    }
    const Eyebrow = ({ children }) => React.createElement("p", { style: { margin: 0, fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.14em", color: "#B45309" } }, children);
    const Title = ({ children }) => React.createElement("p", { style: { margin: "3px 0 0", fontSize: 17, fontWeight: 700, letterSpacing: "-0.02em", color: INK } }, children);
    const Nav = ({ active }) => (React.createElement("div", { style: { marginTop: "auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 3, borderRadius: 12, border: `1px solid ${LINE}`, padding: 4, fontSize: 9, textAlign: "center", color: MUTED } }, ["Início", "Lançar", "Relatórios", "Conta"].map((l) => React.createElement("span", { key: l, style: { borderRadius: 8, padding: "7px 0", background: l === active ? "#DCFCE7" : "transparent", color: l === active ? "#166534" : MUTED, fontWeight: l === active ? 700 : 500 } }, l))));
    function Finger({ x, y, at, T }) {
        const u = clamp((T - at) / 0.35, 0, 1);
        const press = u < 0.5 ? u * 2 : 2 - u * 2;
        return React.createElement("div", { style: { position: "absolute", left: x - 16, top: y - 16, width: 32, height: 32, borderRadius: 999, background: "rgba(34,197,94,0.35)", border: "2px solid rgba(34,197,94,0.9)", transform: `scale(${0.6 + press * 0.6})`, opacity: T >= at && T < at + 0.6 ? 1 : 0 } });
    }
    function Piece() {
        ...
        return (React.createElement("div", { style: { position: "absolute", inset: 0, fontFamily: FONT, color: INK, opacity: fadeOut } },
            React.createElement("div", { style: { position: "absolute", left: 70, top: 40, width: 270, height: 520, borderRadius: 38, background: "#1E293B", padding: 10, boxShadow: "0 30px 70px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(255,255,255,0.08)", transform: `translateY(${phoneScreenShift}px)` } },
                React.createElement("div", { style: { position: "relative", width: "100%", height: "100%", borderRadius: 30, background: "#FFFFFF", overflow: "hidden" } },
                    React.createElement(Screen, { show: between(0, c.Início) },
                        React.createElement(Eyebrow, null, "Fluxo de caixa no celular"),
                        React.createElement(Title, null, "Lan\u00E7ar"),
                        React.createElement("div", { style: { display: "flex", gap: 2, padding: 3, borderRadius: 9, background: "#F1F5F9", fontSize: 10, fontWeight: 600 } },
                            React.createElement("span", { style: { flex: 1, textAlign: "center", padding: 6, borderRadius: 7, background: "#fff", boxShadow: "0 1px 3px rgba(15,23,42,.12)" } }, "Gasto"),
                            React.createElement("span", { style: { flex: 1, textAlign: "center", padding: 6, color: MUTED } }, "Entrada")),
                        React.createElement("div", { style: { borderRadius: 14, border: `1px solid ${LINE}`, padding: "10px 12px" } },
                            React.createElement("p", { style: { margin: 0, fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: ".06em", color: MUTED } }, "Valor"),
                            React.createElement("p", { style: { margin: "3px 0 0", fontFamily: DISPLAY, fontSize: 28, fontWeight: 800, letterSpacing: "-0.03em", color: cents ? INK : "#94A3B8", fontVariantNumeric: "tabular-nums" } }, brl(cents / 100))),
                        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 5 } }, ["Mercado", "Energia", "Transporte", "Saúde", "Delivery", "Lazer", "Cartão", "Outros"].map((l) => React.createElement("span", { key: l, style: { borderRadius: 9, padding: "9px 0", textAlign: "center", fontSize: 9, fontWeight: 600, border: `1px solid ${l === "Mercado" && catOn ? INK : LINE}`, background: l === "Mercado" && catOn ? INK : "#fff", color: l === "Mercado" && catOn ? "#fff" : "#334155" } }, l))),
                        React.createElement("div", { style: { borderRadius: 10, background: GREEN, color: "#052E16", padding: 11, textAlign: "center", fontSize: 12, fontWeight: 700 } }, "Confirmar gasto"),
                        React.createElement("div", { style: { position: "absolute", left: 12, right: 12, bottom: 64, borderRadius: 12, background: INK, color: "#fff", padding: "10px 12px", fontSize: 11, fontWeight: 600, opacity: toast, transform: `translateY(${(1 - toast) * 10}px)` } },
                            "Gasto de R$ 35,90 registrado em Mercado. ",
                            React.createElement("span", { style: { color: "#86EFAC" } }, "Desfazer")),
                        React.createElement(Nav, { active: "Lan\u00E7ar" })),
                    React.createElement(Screen, { show: between(c.Início, c.Exportar) },
                        React.createElement(Eyebrow, null, "Fluxo de caixa no celular"),
                        React.createElement(Title, null, "Resumo financeiro"),
                        React.createElement("div", { style: { borderRadius: 14, background: INK, color: "#fff", padding: 14, display: "flex", flexDirection: "column", gap: 8 } },
                            React.createElement("p", { style: { margin: 0, fontSize: 10, color: "#94A3B8" } }, "Em caixa neste m\u00EAs"),
                            React.createElement("p", { style: { margin: 0, fontFamily: DISPLAY, fontSize: 28, fontWeight: 800, letterSpacing: "-0.03em", fontVariantNumeric: "tabular-nums" } }, brl(saldo)),
                            React.createElement("span", { style: { alignSelf: "flex-start", borderRadius: 999, padding: "4px 9px", fontSize: 10, fontWeight: 600, background: "rgba(34,197,94,.14)", color: "#86EFAC", opacity: M.enter(c.Início + 1.2, c.Início + 1.5)(T) } }, "\u25CF O caixa est\u00E1 respirando."),
                            React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 } },
                                React.createElement("div", { style: { borderRadius: 9, background: "rgba(255,255,255,.06)", padding: 8 } },
                                    React.createElement("p", { style: { margin: 0, fontSize: 9, color: "#94A3B8" } }, "Entradas"),
                                    React.createElement("p", { style: { margin: "3px 0 0", fontSize: 12, fontWeight: 700, color: "#4ADE80" } }, "R$ 3.420,00")),
                                React.createElement("div", { style: { borderRadius: 9, background: "rgba(255,255,255,.06)", padding: 8 } },
                                    React.createElement("p", { style: { margin: 0, fontSize: 9, color: "#94A3B8" } }, "Gastos"),
                                    React.createElement("p", { style: { margin: "3px 0 0", fontSize: 12, fontWeight: 700 } }, "R$ 2.159,20")))),
                        React.createElement("div", { style: { borderRadius: 12, border: `1px solid ${LINE}`, padding: 10, display: "flex", flexDirection: "column", gap: 7 } },
                            React.createElement(Eyebrow, null, "Pra onde est\u00E1 indo"),
                            bars.map(([n, v, col], i) => React.createElement("div", { key: n },
                                React.createElement("div", { style: { display: "flex", justifyContent: "space-between", fontSize: 10, color: "#334155" } },
                                    React.createElement("span", null, n),
                                    React.createElement("b", { style: { fontVariantNumeric: "tabular-nums" } }, brl(v))),
                                React.createElement("div", { style: { height: 5, borderRadius: 999, background: "#F1F5F9", marginTop: 3 } },
                                    React.createElement("div", { style: { height: "100%", borderRadius: 999, background: col, width: `${(v / 950) * 100 * M.enter(c.Início + 0.6 + i * 0.12, c.Início + 1.4 + i * 0.12)(T)}%` } }))))),
                        React.createElement(Nav, { active: "In\u00EDcio" })),
                    React.createElement(Screen, { show: between(c.Exportar, authoredTotal + 1) },
                        React.createElement(Eyebrow, null, "Fluxo de caixa no celular"),
                        React.createElement(Title, null, "Conta"),
                        React.createElement("div", { style: { borderRadius: 14, border: "1px solid #FCD34D", background: "#FFFBEB", padding: 12, display: "flex", flexDirection: "column", gap: 8 } },
                            React.createElement(Eyebrow, null, "Planilha Google"),
                            React.createElement("p", { style: { margin: 0, fontSize: 13, fontWeight: 700 } }, conectando ? "Virada Financeira" : "Conectar sua planilha"),
                            React.createElement("p", { style: { margin: 0, fontSize: 10, lineHeight: 1.5, color: "#475569" } }, conectando ? "Planilha criada no seu Google Drive." : "Seus lançamentos, dívidas e metas vão para uma planilha completa no seu Drive."),
                            React.createElement("div", { style: { borderRadius: 10, background: conectando ? "#DCFCE7" : GREEN, color: conectando ? GREEN_D : "#052E16", padding: 10, textAlign: "center", fontSize: 11, fontWeight: 700 } }, conectando ? (T > c.Exportar + 2.0 ? "✓ Atualizada agora" : "Criando sua planilha…") : "Conectar Google Planilhas")),
                        React.createElement("div", { style: { borderRadius: 12, border: `1px solid ${LINE}`, padding: 10, fontSize: 10, color: MUTED } },
                            React.createElement("b", { style: { color: INK } }, "Dados no seu celular"),
                            React.createElement("br", null),
                            "11 lan\u00E7amentos \u00B7 6 d\u00EDvidas \u00B7 4 metas"),
                        React.createElement(Nav, { active: "Conta" })))),
            React.createElement(Finger, { x: 205, y: 392, at: c.Lançar + 2.15, T: T }),
            React.createElement(Finger, { x: 205, y: 262, at: c.Exportar + 0.75, T: T }),
            React.createElement("div", { style: { position: "absolute", left: flyX - 7, top: flyY - 7, width: 14, height: 14, borderRadius: 999, background: GREEN, boxShadow: "0 0 24px rgba(34,197,94,.9)", opacity: fly > 0 && fly < 1 ? 1 : 0 } }),
            React.createElement("div", { style: { position: "absolute", left: 400, top: 70, width: 680, height: 430, borderRadius: 16, background: "#1E293B", padding: "12px 12px 0", boxShadow: "0 30px 70px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(255,255,255,0.08)" } },
                React.createElement("div", { style: { position: "relative", width: "100%", height: "100%", borderRadius: "8px 8px 0 0", background: "#0B1220", overflow: "hidden" } },
                    React.createElement("div", { style: { position: "absolute", inset: 0, background: "#FFFFFF", opacity: laptopOn } },
                        React.createElement("div", { style: { height: 34, background: "#F8FAFC", borderBottom: `1px solid ${LINE}`, display: "flex", alignItems: "center", gap: 10, padding: "0 12px" } },
                            React.createElement("span", { style: { width: 16, height: 20, borderRadius: 3, background: "#22A565" } }),
                            React.createElement("span", { style: { fontSize: 11, fontWeight: 600, color: "#334155", whiteSpace: "nowrap" } }, "Virada Financeira \u2014 Planilhas Google"),
                            React.createElement("span", { style: { marginLeft: "auto", fontSize: 9, color: MUTED, border: `1px solid ${LINE}`, borderRadius: 999, padding: "2px 8px", whiteSpace: "nowrap" } }, "Salvo no Drive")),
                        React.createElement("div", { style: { height: 20, background: "#FFFFFF", borderBottom: `1px solid ${LINE}`, display: "flex", gap: 14, padding: "0 12px", alignItems: "center", fontSize: 9, color: MUTED } }, ["Arquivo", "Editar", "Ver", "Inserir", "Formatar", "Dados"].map((m) => React.createElement("span", { key: m }, m))),
                        React.createElement("div", { style: { padding: 10, display: "flex", flexDirection: "column", gap: 8 } },
                            React.createElement("div", { style: { borderRadius: 6, background: INK, color: "#fff", padding: "8px 12px", borderBottom: `2px solid ${GREEN}`, transform: `translateY(${(1 - banner) * -10}px)`, opacity: banner } },
                                React.createElement("p", { style: { margin: 0, fontSize: 12, fontWeight: 700, letterSpacing: "-0.01em" } }, "C\u00D3DIGO DA VIRADA \u2022 BASE FINANCEIRA CLARA E ESTRUTURADA"),
                                React.createElement("p", { style: { margin: "2px 0 0", fontSize: 8, color: "#94A3B8" } }, "Atualizado agora pelo app")),
                            React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 } }, kpis.map(([l, v, hi], i) => React.createElement("div", { key: l, style: { borderRadius: 6, border: `1px solid ${hi ? "#86EFAC" : LINE}`, background: hi ? "#DCFCE7" : "#fff", padding: "8px 12px", opacity: kpi(i), transform: `scale(${kpiPop(i)})` } },
                                React.createElement("p", { style: { margin: 0, fontSize: 8, fontWeight: 700, color: hi ? GREEN_D : MUTED, letterSpacing: ".04em" } }, l),
                                React.createElement("p", { style: { margin: "3px 0 0", fontFamily: DISPLAY, fontSize: 18, fontWeight: 800, letterSpacing: "-0.02em", color: hi ? GREEN_D : INK, fontVariantNumeric: "tabular-nums" } }, brl(count(v, i)))))),
                            React.createElement("div", { style: { border: `1px solid ${LINE}`, borderRadius: 6, overflow: "hidden", opacity: M.enter(P + 1.2, P + 1.5)(T) } },
                                React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1.1fr .8fr 1.2fr 1fr", background: "#F1F5F9", borderBottom: "1px solid #CBD5E1", fontSize: 8, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: ".04em" } }, ["Data", "Tipo", "Categoria", "Valor"].map((h) => React.createElement("span", { key: h, style: { padding: "6px 10px" } }, h))),
                                rows.map(([d, t, cat, v], i) => React.createElement("div", { key: d + cat, style: { display: "grid", gridTemplateColumns: "1.1fr .8fr 1.2fr 1fr", fontSize: 10, color: "#334155", background: i % 2 ? "#F8FAFC" : "#fff", borderBottom: `1px solid ${LINE}`, opacity: rowIn(i), transform: `translateX(${(1 - rowIn(i)) * -8}px)` } },
                                    React.createElement("span", { style: { padding: "5px 10px" } }, d),
                                    React.createElement("span", { style: { padding: "4px 10px" } },
                                        React.createElement("span", { style: { borderRadius: 999, padding: "1px 7px", fontSize: 8, fontWeight: 700, background: t === "Entrada" ? "#DCFCE7" : "#F1F5F9", color: t === "Entrada" ? GREEN_D : "#334155" } }, t)),
                                    React.createElement("span", { style: { padding: "5px 10px" } }, cat),
                                    React.createElement("span", { style: { padding: "5px 10px", fontWeight: 700, color: GREEN_D, fontVariantNumeric: "tabular-nums" } }, brl(v))))),
                            React.createElement("div", { style: { display: "flex", gap: 4, opacity: M.enter(P + 2.2, P + 2.5)(T) } }, ["Dashboard", "Lançamentos", "Receitas", "Despesas", "Dívidas", "Metas", "Fluxo de Caixa", "Resumo Mensal", "Como usar"].map((t, i) => React.createElement("span", { key: t, style: { borderRadius: 5, padding: "3px 8px", fontSize: 8, fontWeight: 600, background: i === 0 ? INK : "#fff", color: i === 0 ? "#fff" : "#334155", border: `1px solid ${i === 0 ? INK : LINE}` } }, t))))),
                    React.createElement("div", { style: { position: "absolute", inset: 0, display: "grid", placeItems: "center", opacity: 1 - laptopOn, color: "#334155", fontSize: 12 } }, "aguardando o app\u2026"))),
            React.createElement("div", { style: { position: "absolute", left: 380, top: 500, width: 720, height: 14, borderRadius: "0 0 14px 14px", background: "#334155" } }),
    ...
    function ViradaDemo() {
        ...
        return (React.createElement("div", { ref: ref, style: { position: "relative", width: "100%", height: "100%" } },
            React.createElement(CompositionStage, { width: 1160, height: 600, scenes: window.OM_SCENES, playback: window.OM_PLAYBACK, bg: "#0F172A", background: "#0F172A", persistKey: "virada-demo" },
