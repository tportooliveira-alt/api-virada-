...
export function removeDebtRow(userId: string, id: string) {
  writeRows(
    "debts",
    readRows("debts").filter((row) => !(row.user_id === userId && row.id === id)),
  );
  writeRows(
    "accountsPayable",
    readRows("accountsPayable").filter((row) => !(row.user_id === userId && row.id === id)),
  );
}

export function removeGoalRow(userId: string, id: string) {
  writeRows(
    "goals",
    readRows("goals").filter((row) => !(row.user_id === userId && row.id === id)),
  );
}

export function addEstorno(
  userId: string,
  original: {
    id: string;
    type: "expense" | "income";
  ...
  const row: Row = {
    ...
    nature: original.nature ?? "essencial",
    scope: original.scope ?? "casa",
    source: "app",
    date: original.date,
    created_at: now(),
    sync_status: "pending",
...
// ─── Aliases para compatibilidade com API routes ───────────────────────────

/** Alias de findUserById — retorna o usuário completo */
export function getUserById(userId: string) {
  const row = readRows("users").find((u) => u.id === userId);
  if (!row) return null;
  return {
    id: String(row.id),
    email: String(row.email),
    fullName: String(row.full_name ?? ""),
    role: String(row.role ?? "customer") as "customer" | "admin",
    plan: String(row.plan ?? "basic") as "basic" | "premium",
    sheetId: row.sheet_url ? String(row.sheet_url) : null,
    sheetEmail: null as string | null,
  };
}

/** Atualiza o sheetId (URL da planilha) do usuário */
export function saveUser(update: { id: string; sheetId: string | null; [key: string]: unknown }) {
  ensureStore();
  const allRows = readRows("users");
  const idx = allRows.findIndex((u) => u.id === update.id);
  if (idx === -1) return;
  allRows[idx] = {
    ...allRows[idx],
    sheet_url: update.sheetId ?? "",
    updated_at: now(),
  };
  // Re-escreve o arquivo CSV
  const header = headers.users.join(",");
  const csvRows = allRows.map((r) =>
    headers.users.map((h) => JSON.stringify(r[h] ?? "")).join(","),
  );
  const csvPath = path.join(dataDir, files.users);
