...
export function MissaoDoDia({ mission, done, onToggle }: MissaoDoDiaProps) {
  return (
    <section
      className={`rounded-lg border p-5 shadow-card transition-colors ${
        done
          ? "border-green-500/40 bg-green-500/10"
          : "border-virada-gold/30 bg-white"
      }`}
    >
      {/* Header row */}
      ...
      {/* Mission content */}
      <h2 className="mt-3 text-xl font-semibold text-ink-900">{mission.title}</h2>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        {done ? (
          <>
            ...
            <Link
              href="/app/missoes"
              className="flex items-center gap-1 text-sm text-virada-gray hover:text-ink-900"
            >
          ...
          <>
            <button
              ...
              className="animate-pulse rounded-md bg-virada-gold px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-amber-400 hover:animate-none focus:outline-none focus:ring-2 focus:ring-virada-gold focus:ring-offset-2 focus:ring-offset-virada-bg"
            >
              Marcar como feita
            </button>
            <Link
