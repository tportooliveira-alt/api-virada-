...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-virada-bg text-ink-900">
      <section className="mx-auto grid min-h-[92vh] w-full max-w-6xl items-center gap-10 px-5 py-8 md:grid-cols-[1.02fr_0.98fr] lg:px-8">
        <div className="space-y-7">
          <div className="inline-flex items-center gap-2 rounded-full border border-virada-green/30 bg-virada-green/10 px-3 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-green-700">
            <Banknote className="h-4 w-4" />
            Educacao financeira pratica
          </div>

          <div className="space-y-5">
            <h1 className="max-w-3xl text-4xl font-semibold leading-[1.05] text-ink-900 sm:text-5xl lg:text-6xl">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <a
              href="/app/inicio"
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-green-500 px-5 py-3 text-sm font-bold text-green-900 transition hover:bg-green-400"
            >
              Quero minha virada <ArrowRight className="h-4 w-4" />
            </a>
            <Link
              href="/app/inicio"
              className="inline-flex items-center justify-center gap-2 rounded-lg border border-ink-200 px-5 py-3 text-sm font-semibold text-ink-900 transition hover:bg-ink-100"
            >
      ...
      <section className="border-y border-virada-line bg-ink-50">
        <div className="mx-auto grid max-w-6xl gap-4 px-5 py-8 md:grid-cols-3 lg:px-8">
          {pillars.map((item) => (
            <article key={item.title} className="rounded-lg border border-virada-line bg-white p-5">
              <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-lg bg-virada-green/10 text-green-700">
                {item.icon}
              </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-5 py-12 md:grid-cols-[0.9fr_1.1fr] lg:px-8">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-semibold leading-tight text-ink-900">Nao e promessa magica. E direcao, registro e constancia.</h2>
        </div>
